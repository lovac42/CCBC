...
go to the Plugins>Add-ons config menu, and disable or delete the add-on.
    ...
    def _readManifestFile(self, zfile):
        try:
            ...
            for key, attrs in self._manifest_schema.items():
                ...
                manifest[key] = val
        except (KeyError, AssertionError, json.decoder.JSONDecodeError): #Py3.5
        # except (KeyError, AssertionError, ValueError): #Py3.4
...
#MODS FROM: https://github.com/ijgnd/anki__misc/blob/master/addons_window_filter_bar_for_21.py
    def addFilterBar(self):
        ...
        self.mw.progress.timer(1,self.filterbar.setFocus,False) #https://stackoverflow.com/a/52858926
